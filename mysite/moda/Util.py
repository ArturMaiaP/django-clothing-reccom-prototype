...
def get_df_treino(df_teste, list_rel, list_irrel):
    ...
    relevante_idx = df_teste.loc[df_teste['image_name'].isin(list_rel)].index
    df_relevante = df_teste.loc[relevante_idx, :]
    df_relevante['Class'] = 1.0

    list_irrel = [item.replace("/static/","") for item in list_irrel]
    irrelevante_idx = df_teste.loc[df_teste['image_name'].isin(list_irrel)].index
...
def update_df_teste(df_treino, df_teste):
    for name in df_treino.image_name:
        df_teste = df_teste[df_teste.image_name != name]
